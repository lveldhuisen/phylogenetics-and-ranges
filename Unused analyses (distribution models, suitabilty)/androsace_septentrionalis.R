...
#get taxon key 
key1 <- name_backbone(name='Androsace septentrionalis', kingdom='plants', rank = "species")

key1<-name_backbone(name='Androsace.septentrionalis',rank='species')$usageKey
...
occ_download(
  ...
  pred("taxonKey", 3169326),
  format = "SIMPLE_CSV",
  user="leah.veldhuisen", 
  pwd="Columbia2305", 
  email="leah.veldhuisen@gmail.com"
)
d <- occ_download_get('0016239-231002084531237') %>%
  occ_download_import()
...
AndSep_occ <- df1[df1$species == "Androsace.septentrionalis",]
...
mask_of_NA <- shapefile("bound_p/boundaries_p_2021_v3.shp") ##dont run this line
...
points_androsace <- data.frame(AndSep_occ[1:868,c("decimalLongitude", "decimalLatitude")])
AndSep_cell_id <- cellFromXY(subset(bioclim_NA,1), points_androsace)
...
s.class(pca_NA$li[,1:2],fac = factor(rownames(bioclim_NA_df)
                                     %in% AndSep_cell_id, levels = c("FALSE","TRUE"),
                                     labels=c("background","AndSep")), col = c("red","blue"),
...
AndSep_occ_df <- as.data.frame(AndSep_occ)

resp_var <- rep(1,nrow(AndSep_occ))
...
###formatting data############
AndSep_data <- BIOMOD_FormatingData(
  resp.var = rep(1, nrow(AndSep_occ)), 
  expl.var = env_vars_NA_sub_test,
  resp.xy = AndSep_occ[,c('decimalLongitude','decimalLatitude')],
  resp.name = "Androsace.septentrionalis",
  PA.nb.rep = 3,
  PA.nb.absences = 500, 
  PA.strategy = 'random',
  filter.raster = TRUE
...
AndSep_occ[,c('decimalLongitude','decimalLatitude')]

AndSep_data

###make table for PAs#############
PA <- bm_PseudoAbsences( resp.var = rep(1, nrow(AgoGla_occ)), 
                         expl.var = env_vars_NA_sub_test, 
                         nb.rep=3, 
                         strategy="random", 
                         nb.absences=500, 
...
AndSep_opt <- BIOMOD_ModelingOptions(
...
###running models##############
AndSep_models <- BIOMOD_Modeling(AndSep_data, 
                                 models = c("GLM","GBM","RF","GAM"),
                                 bm.options= AndSep_opt, 
                                 CV.nb.rep = 4, 
                                 CV.perc =0.8,
                                 var.import = 3, 
                                 CV.do.full.models = F, 
...
###get model scores#######
AndSep_models_scores <- get_evaluations(AndSep_models)
dim(AndSep_models_scores)
dimnames(AndSep_models_scores)

bm_PlotEvalMean(bm.out = AndSep_models)
...
#ensemble modeling-----------------
AndSep_ensemble_model <- BIOMOD_EnsembleModeling(AndSep_models, 
                                                 em.by = 'all',
                                                 metric.select = 'TSS',
                                                 ...
                                                 var.import = 0)
AS_ensemble_model_scores <- get_evaluations(AndSep_ensemble_model)
AS_ensemble_model_scores

#make maps using ensemble modeling------------------------------
AndSep_models_proj_current <- BIOMOD_Projection(bm.mod = AndSep_models, 
...
AndSep_ensemble_models_proj_current <- BIOMOD_EnsembleForecasting(bm.em = AndSep_ensemble_model,
                                                                  bm.proj =AndSep_models_proj_current,
                                                                  proj.name = 'test',
                                                                  metric.binary = "TSS",
                                                                  output.format = ".img",
                                                                  do.stack = FALSE,
...
plot(AndSep_models_proj_current)
writeRaster(AndSep_ensemble_models_proj_current, filename = "/Users/leahvedlhuisen/Library/CloudStorage/OneDrive-UniversityofArizona/Arizona PhD/Research/Chapter 2/distribution modeling/Andsep.img", format = "HFA" )

#play with plotting--------
install.packages("arulesViz")
library(arulesViz)

plot(AgoGla_ensemble_models_proj_current,
     coord = AgoAur_occ_df,
     plot.output = 'facet',
     do.plot = TRUE,
     std = TRUE,
...
models.needed <- get_kept_models(AndSep_ensemble_model)
formal_pred <- BIOMOD_Projection(bm.mod = AndSep_models,
                                 new.env = env_values,
                                 proj.name = "current",
                                 new.env.xy = xy,
                                 models.chosen = 'all',
