...
#get taxon key 
key1 <- name_backbone(name='Agoseris glauca', kingdom='plants', rank = "species")

key1<-name_backbone(name='Agoseris.glauca',rank='species')$usageKey
...
d <- occ_download_get('0016202-231002084531237') %>%
  occ_download_import()
...
AgoGla_occ <- df1[df1$species == "Agoseris.glauca",]
...
points_agoseris <- data.frame(AgoGla_occ[1:868,c("decimalLongitude", "decimalLatitude")])
AgoGla_cell_id <- cellFromXY(subset(bioclim_NA,1), points_agoseris)
...
s.class(pca_NA$li[,1:2],fac = factor(rownames(bioclim_NA_df)
                                     %in% AgoGla_cell_id, levels = c("FALSE","TRUE"),
                                     labels=c("background","AgoGla")), col = c("red","blue"),
...
AgoGla_occ_df <- as.data.frame(AgoGla_occ)

resp_var <- rep(1,nrow(AgoGla_occ))
...
###formatting data############
AgoGla_data <- BIOMOD_FormatingData(
  resp.var = rep(1, nrow(AgoGla_occ)), 
  expl.var = env_vars_NA_sub_test,
  resp.xy = AgoGla_occ[,c('decimalLongitude','decimalLatitude')],
  resp.name = "Agoseris.glauca",
  PA.nb.rep = 3,
  PA.nb.absences = 500, 
  PA.strategy = 'random',
  filter.raster = TRUE
...
AgoGla_occ[,c('decimalLongitude','decimalLatitude')]
...
AgoGla_opt <- BIOMOD_ModelingOptions(
...
###running models##############
AgoGla_models <- BIOMOD_Modeling(AgoGla_data, 
                                 models = c("GLM","GBM","RF","GAM"),
                                 bm.options= AgoGla_opt, 
                                 CV.nb.rep = 4, 
                                 CV.perc =0.8,
                                 var.import = 3, 
                                 CV.do.full.models = F, 
...
###get model scores#######
AgoGla_models_scores <- get_evaluations(AgoGla_models)
dim(AgoGla_models_scores)
dimnames(AgoGla_models_scores)

bm_PlotEvalMean(bm.out = AgoGla_models)
...
#ensemble modeling-----------------
AgoGla_ensemble_model <- BIOMOD_EnsembleModeling(AgoGla_models, 
                                                 em.by = 'all',
                                                 metric.select = 'TSS',
                                                 ...
                                                 var.import = 0)
AG_ensemble_model_scores <- get_evaluations(AgoGla_ensemble_model)
AG_ensemble_model_scores

#make maps using ensemble modeling------------------------------
AgoGla_models_proj_current <- BIOMOD_Projection(bm.mod = AgoGla_models, 
...
AgoGla_ensemble_models_proj_current <- BIOMOD_EnsembleForecasting(bm.em = AgoGla_ensemble_model,
                                                                  bm.proj =AgoGla_models_proj_current,
                                                                  proj.name = 'test',
                                                                  metric.binary = "TSS",
                                                                  output.format = ".img",
                                                                  do.stack = FALSE,
...
plot(AgoGla_models_proj_current)
writeRaster(AgoGla_ensemble_models_proj_current, filename = "/Users/leahvedlhuisen/Library/CloudStorage/OneDrive-UniversityofArizona/Arizona PhD/Research/Chapter 2/distribution modeling/agogla.img", format = "HFA" )
...
models.needed <- get_kept_models(AgoGla_ensemble_model)
formal_pred <- BIOMOD_Projection(bm.mod = AgoGla_models,
                                 new.env = env_values,
                                 proj.name = "current",
                                 new.env.xy = xy,
                                 models.chosen = 'all',
