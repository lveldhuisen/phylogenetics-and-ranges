...
#get taxon key 
name_backbone("Aquilegia coerulea")
key1<-name_backbone(name='Aquilegiacoerulea',rank='species')$usageKey
...
occ_download_wait('0025989-231002084531237')
d <- occ_download_get('0025989-231002084531237') %>%
  occ_download_import()
...
AquCoe_occ <- df1[df1$species == "Aquilegia.coerulea",]
...
points_aquilegia <- data.frame(AquCoe_occ[1:2555,c("decimalLongitude", "decimalLatitude")])
AquCoe_cell_id <- cellFromXY(subset(bioclim_NA,1), points_aquilegia)
...
s.class(pca_NA$li[,1:2],fac = factor(rownames(bioclim_NA_df)
                                     %in% AquCoe_cell_id, levels = c("FALSE","TRUE"),
...
s.class(pca_NA_soil$li[,1:2],fac = factor(rownames(soil_NA_df)
                                          %in% AquCoe_cell_id, levels = c("FALSE","TRUE"),
                                          labels=c("background","AquCoe")), col = c("red","blue"),
        csta = 0, cellipse = 2, cpoint = .3, pch = 16)
mtext("(a)",side = 3, line = 3, adj = 0)
...
AquCoe_occ_df <- as.data.frame(AquCoe_occ)

resp_var <- rep(1,nrow(AquCoe_occ))
...
###formatting data############
AquCoe_data <- BIOMOD_FormatingData(
  resp.var = rep(1, nrow(AquCoe_occ)), 
  expl.var = env_vars_NA_sub_test,
  resp.xy = AquCoe_occ[,c('decimalLongitude','decimalLatitude')],
  resp.name = "Aquilegia.coerulea",
  PA.nb.rep = 3,
  PA.nb.absences = 500, 
  PA.strategy = 'random',
  filter.raster = TRUE
...
AquCoe_occ[,c('decimalLongitude','decimalLatitude')]

AquCoe_data

###make table for PAs#############
PA <- bm_PseudoAbsences( resp.var = rep(1, nrow(AquCoe_occ)), 
                         expl.var = env_vars_NA_sub_test, 
                         nb.rep=3, 
                         strategy="random", 
                         nb.absences=500, 
...
AquCoe_opt <- BIOMOD_ModelingOptions(
...
###running models##############
AquCoe_models <- BIOMOD_Modeling(AquCoe_data, 
                                 models = c("GLM","GBM","RF","GAM"),
                                 bm.options= AquCoe_opt, 
                                 CV.nb.rep = 4, 
                                 CV.perc =0.8,
                                 var.import = 3, 
                                 CV.do.full.models = F, 
...
###get model scores#######
AquCoe_models_scores <- get_evaluations(AquCoe_models)
dim(AquCoe_models_scores)
dimnames(AquCoe_models_scores)

bm_PlotEvalMean(bm.out = AquCoe_models)

###testing different variables importance - did not do########
AquCoe_models_var_import <- get_variables_importance(AquCoe_models)
apply(AquCoe_models_var_import, c(1,2), mean)

bm_VariablesImportance( bm.model = AquCoe_models, 
                        expl.var = env_vars_NA_sub_test, 
                        method="full_rand", 
                        nb.rep=1, 
                        do.progress=TRUE)
...
#ensemble modeling-----------------
AquCoe_ensemble_model <- BIOMOD_EnsembleModeling(AquCoe_models, 
                                                 em.by = 'all',
                                                 metric.select = 'TSS',
                                                 ...
                                                 var.import = 0)
AC_ensemble_model_scores <- get_evaluations(AquCoe_ensemble_model)
AC_ensemble_model_scores

#make maps using ensemble modeling------------------------------
AquCoe_models_proj_current <- BIOMOD_Projection(bm.mod = AquCoe_models, 
...
AquCoe_ensemble_models_proj_current <- BIOMOD_EnsembleForecasting(bm.em = AquCoe_ensemble_model,
                                                                  bm.proj =AquCoe_models_proj_current,
                                                                  proj.name = 'test',
                                                                  metric.binary = "TSS",
                                                                  output.format = ".img",
                                                                  do.stack = FALSE,
...
plot(AquCoe_models_proj_current)
writeRaster(AquCoe_ensemble_models_proj_current, filename = "/Users/leahvedlhuisen/Library/CloudStorage/OneDrive-UniversityofArizona/Arizona PhD/Research/Chapter 2/distribution modeling/aqucoe.img", format = "HFA" )
...
gps_point <- data.frame(x = -107.0153, y = 38.5738)
gps_point <- as.data.frame(gps_point)
extent(env_layer4)

###test point #######
test_point <- data.frame(x = -106.5843, y=38.5350)
...
models.needed <- get_kept_models(AquCoe_ensemble_model)
formal_pred <- BIOMOD_Projection(bm.mod = AquCoe_models,
                                 new.env = env_values,
                                 proj.name = "current",
                                 new.env.xy = xy,
                                 models.chosen = 'all',
